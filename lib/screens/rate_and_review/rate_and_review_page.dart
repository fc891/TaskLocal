...
// upload images to firebase
  Future<List<String>> _imageUpload(List<XFile> images) async {
    ...
    for (XFile image in images) {
      ...
      String fileName =
          'reviews/${DateTime.now().millisecondsSinceEpoch}_${image.name}';
  ...
  Widget _imageDisplay() {
    return _images.isNotEmpty
        ...
        : Text('No images selected',
            style: TextStyle(color: Color.fromARGB(255, 99, 97, 97)));
  }
  ...
  Future<Map<String, dynamic>?> fetchTaskerInfo(String taskerEmail) async {
    try {
      final DocumentSnapshot taskerDoc = await FirebaseFirestore.instance
          .collection(
              'Taskers') // Make sure this is your correct collection name
          .doc(taskerEmail)
          .get();
...
// submit button to store information
  Future<void> _submitReview() async {
    ...
    //Recalculate rating, add one to number of total reviews
    double overallRating;
    double averageRatingNew;
    int numberReviews;
    var collection = FirebaseFirestore.instance.collection('Taskers');
    var docSnapshot = await collection.doc(widget.taskerEmail).get();
    Map<String, dynamic> data = docSnapshot.data()!;
    numberReviews = data['numberReviews'] + 1;
    overallRating = data['overallRating'] + _rating;
    averageRatingNew = double.parse((overallRating / numberReviews).toStringAsFixed(1));
    FirebaseFirestore.instance
        .collection('Taskers')
        .doc(widget.taskerEmail)
        .set({'rating': averageRatingNew}, SetOptions(merge: true)).then(
            (value) {
      averageRatingNew = data['rating'];
    });
    FirebaseFirestore.instance
        .collection('Taskers')
        .doc(widget.taskerEmail)
        .set({'overallRating': overallRating}, SetOptions(merge: true)).then(
            (value) {
    ...
    FirebaseFirestore.instance
        .collection('Taskers')
        .doc(widget.taskerEmail)
        .set({'numberReviews': numberReviews},
    ...
    if (_taskTypeController.text.isEmpty ||
        _rating == 0 ||
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          title: Text('Missing Information',
              style: TextStyle(color: Colors.white)),
          content: Text('Please make sure all areas of field are filled out.',
              style: TextStyle(color: Colors.white)),
          actions: <Widget>[
            TextButton(
    ...
    if (customerInfo == null) {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          backgroundColor: Theme.of(context).colorScheme.tertiary,
          title: Text('Error', style: TextStyle(color: Colors.white)),
          content: Text('Could not load your information. Try Again or Relog.',
              style: TextStyle(color: Colors.white)),
          actions: <Widget>[
            TextButton(
              child: Text('OK', style: TextStyle(color: Colors.white)),
    ...
    try {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          backgroundColor: Theme.of(context).colorScheme.tertiary,
          title:
              Text('Review Submitted', style: TextStyle(color: Colors.white)),
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          backgroundColor: Theme.of(context).colorScheme.tertiary,
          title: Text('Error', style: TextStyle(color: Colors.white)),
          content: Text('Error occured when attempting to submit. Try Again.',
              style: TextStyle(color: Colors.white)),
          actions: <Widget>[
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
                        ...
                        return Row(
                          children: [
                            CircleAvatar(
                              ...
                              backgroundImage: NetworkImage(taskerInfo?[
                                      "profile picture"] ??
                              ...
                              radius: 30,
                              child: taskerInfo?["profile picture"] == null
                                  ? Icon(Icons.person)
                                  : null,
                            ),
                            SizedBox(width: 10),
